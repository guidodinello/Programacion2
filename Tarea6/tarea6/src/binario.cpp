...
TBinario avlABinario(TAvl a) {
	...
	if (!estaVacioAvl(a)) {
		...
		if (!estaVacioAvl(izqAvl(a)))
			res -> izq = avlABinario(izqAvl(a));
		if (!estaVacioAvl(derAvl(a)))
			res -> der = avlABinario(derAvl(a));
	}
	return res;
}

static TBinario binarioAux(ArregloNats elems, int min, int max) {
	TBinario res = NULL;
	if (max >= min) {
		nat prom = (max + min)/2;
		res = new _rep_binario;
		res -> elem = crearInfo(elems[prom],0);
		res -> izq = binarioAux(elems, min, prom -1);
		res -> der = binarioAux(elems, prom +1, max);
	}
	return res;
}
TBinario binarioBalanceado(ArregloNats elems, nat n) {
	TBinario b = binarioAux(elems, 0, n-1);
	return b;
}
