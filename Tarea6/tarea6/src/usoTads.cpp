...
// A intersec B = A\(A\B) = B\(B\A)
	TConjunto a_menos_b = diferenciaDeConjuntos(c1,c2); // O(|c1|+|c2|)
	TConjunto res = diferenciaDeConjuntos(c1, a_menos_b); // O(|c2|+|c1| - |c2|)
	liberarConjunto(a_menos_b);
	return res;
}
